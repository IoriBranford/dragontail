...
function Gauge:setBarColor(color)
	local gauge = self.gauge
	self.gaugecolor = color
    gauge.barred, gauge.bargreen, gauge.barblue, gauge.baralpha = Color.unpack(color)
end
...
function Gauge:start(scene)
	...
	self:setBarColor(self.gaugecolor or "ffffffff")
