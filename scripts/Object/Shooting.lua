...
local firedshots = {}
setmetatable(firedshots, { __mode="v" })
...
function Shooting.unitFire(unit)
    if unit.canFire and not unit:canFire() then
        return
    end
...
function Shooting.setFireMulti(unit, shape, count, size)
    if shape == "fan" then
        unit.firemulti = count
        unit.firemultishape = shape
        unit.firefanarc = size
    elseif shape == "ring" then
        unit.firemulti = count
        unit.firemultishape = shape
        unit.fireringradius = size
    elseif shape == "row" then
        unit.firemulti = count
        unit.firemultishape = shape
        unit.firerowlength = size
    end
end
