<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.11" tiledversion="1.12.2" name="cavedoor2-diagonal" tilewidth="64" tileheight="128" tilecount="4" columns="4" objectalignment="left">
 <editorsettings>
  <export target="cavedoor2-diagonal.lua" format="lua"/>
 </editorsettings>
 <image source="cavedoor2-diagonal.png" width="256" height="128"/>
 <tile id="1" type="bandit-cave-door">
  <objectgroup draworder="index" id="2">
   <object id="1" x="0" y="64">
    <properties>
     <property name="collidable" type="bool" value="true"/>
    </properties>
    <polygon points="0,0 64,64 62.7484,46.7484 16,0"/>
   </object>
  </objectgroup>
 </tile>
 <tile id="2">
  <properties>
   <property name="name" value="collapse"/>
  </properties>
 </tile>
</tileset>
